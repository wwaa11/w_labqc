...
export default function AssetTypesMain() {
    ...
    const [assetTypeToDelete, setAssetTypeToDelete] = useState<any>(null);
    ...
    const handleSearch = () => {
        setIsLoading(true);
        router.get(route('asset-types.main'), { search: input }, {
            preserveState: true,
            replace: true,
    ...
    const handleDelete = (assetType: any) => {
    ...
    const confirmDelete = () => {
        if (assetTypeToDelete) {
            router.delete(route('asset-types.destroy', assetTypeToDelete.id));
        }
    ...
    const handleEdit = (id: number) => {
        router.get(route('asset-types.edit', id));
    };

    const handleCreate = () => {
        router.get(route('asset-types.create'));
    };
    ...
    // Filtered asset types based on search
    const filteredAssetTypes = useMemo(() => {
        if (!search) return assetTypes;
        return assetTypes.filter((assetType: any) =>
            assetType.asset_type_name.toLowerCase().includes(search.toLowerCase()) ||
            (assetType.asset?.name && assetType.asset.name.toLowerCase().includes(search.toLowerCase()))
        );
    }, [assetTypes, search]);
    ...
    const stats = useMemo(() => ({
        ...
        active: assetTypes.filter((at: any) => !at.is_deleted).length,
        deleted: assetTypes.filter((at: any) => at.is_deleted).length,
        withAssets: assetTypes.filter((at: any) => at.asset).length,
    }), [assetTypes]);
    ...
    const AssetTypeCard = ({ assetType }: any) => (
        <Card
            sx={{
                height: '100%',
                transition: 'all 0.3s ease',
        ...
        >
            <CardContent sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                    ...
                    <Chip
                        label={assetType.is_deleted ? 'Deleted' : 'Active'}
                        size="small"
                        color={assetType.is_deleted ? 'error' : 'success'}
                        icon={assetType.is_deleted ? <CancelIcon /> : <CheckCircleIcon />}
                        sx={{ fontWeight: 500 }}
                    />
                </Box>
                ...
                <Box sx={{ display: 'flex', gap: 1, mt: 3, pt: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
                    <Tooltip title="Edit Asset Type">
                        <IconButton
                            size="small"
                            onClick={() => handleEdit(assetType.id)}
                            sx={{
                    ...
                    <Tooltip title="Delete Asset Type">
                        <IconButton
                            size="small"
    ...
    return (
        <DashboardLayout>
            <Head title="Asset Types Management" />
            ...
            >
                ...
                <DialogContent>
                    ...
                    <Typography variant="body1">
                        ...
                        <strong>"{assetTypeToDelete?.asset_type_name}"</strong>?
                    </Typography>
                    {assetTypeToDelete?.asset && (
                        <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                            This will also remove the association with asset "{assetTypeToDelete.asset.name}".
                        </Typography>
                    )}
                </DialogContent>
